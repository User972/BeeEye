...
public sealed class ExplainabilityApiTests(IntegrationTestFactory factory)
{
    ...
    // ---------------------------------------------------------------- frozen decision records (S3)

    /// <summary>
    /// Inserts one frozen recommendation, as a generation run does, and returns its unique id — the key
    /// a Decision Log record is explained by. Seeded rather than discovered from a live endpoint because
    /// this kind explains persisted history, not the current analysis.
    /// </summary>
    private async Task<Guid> SeedRecordAsync()
    {
        ...
        var recommendation = new Recommendation
        {
            ...
            IdempotencyKey = $"explain-record|{Guid.NewGuid()}",
    ...
    [Fact]
    public async Task A_frozen_decision_record_explains_itself_by_its_unique_id()
    {
        var id = await SeedRecordAsync();

        using var executive = Executive("exec-decision-record");
        var payload = await ExplainAsync(executive.CreateClient(), "decision-record", id.ToString());

        var explanation = payload.GetProperty("explanation");
        Assert.Equal("ES 350 ZX · Pearl White", explanation.GetProperty("title").GetString());
        Assert.StartsWith("Decision Log", explanation.GetProperty("module").GetString(), StringComparison.Ordinal);

        // A Medium-confidence recommendation, labelled as a recommendation (it is one, frozen).
        Assert.Equal("recommendation", explanation.GetProperty("label").GetString());
        Assert.Equal("Medium", explanation.GetProperty("confidence").GetProperty("band").GetString());

        // The reader is already in the Decision Log looking at this exact record, so the drawer shows
        // no redundant workflow footer — ownership is null by design.
        Assert.Equal(JsonValueKind.Null, explanation.GetProperty("ownership").ValueKind);
    }

    [Fact]
    public async Task A_decision_record_id_matching_nothing_is_a_not_found()
    {
        using var executive = Executive("exec-record-missing");
        var response = await executive.CreateClient()
            .GetAsync(Query("decision-record", Guid.NewGuid().ToString()));

        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }

    [Fact]
    public async Task A_decision_record_reference_that_is_not_an_id_is_a_not_found_not_a_500()
    {
        // A rule id like "D-INV-1" is not a record id: the provider returns null rather than throwing,
        // so a malformed reference is a clean 404, never a gap or an error.
        using var executive = Executive("exec-record-malformed");
        var response = await executive.CreateClient().GetAsync(Query("decision-record", "D-INV-1"));
    ...
    public async Task An_unknown_kind_is_refused_with_the_kinds_that_are_registered()
    {
        ...
        foreach (var kind in new[]
                 {
                     "brief", "configuration", "decision", "decision-record", "forecast-scope",
                     "inventory-unit", "order-configuration", "part", "procurement-item", "service-model",
                 })
        {
    ...
    [Fact]
    public async Task One_persons_candid_feedback_is_not_disclosed_to_another_viewer()
    {
        const string subjectRef = "private-note-subject";
        const string candidNote = "This safety-stock assumption is wrong.";

        // An analyst leaves a candid note.
        using var author = Analyst("analyst-private-note");
        await author.CreateClient().SendAsync(
            Post(FeedbackUrl, Feedback("brief", subjectRef, "Incorrect", candidNote), NewKey()));

        // A different viewer reads the same explanation. The drawer only ever surfaces the reader's own
        // verdict, so the author's note and id must not cross the wire at all — the endpoint returns the
        // caller's own feedback, never everyone's.
        using var other = Executive("exec-other-viewer");
        var explained = await ExplainAsync(other.CreateClient(), "brief", subjectRef);

        Assert.DoesNotContain(
            explained.GetProperty("feedback").EnumerateArray(),
            f => f.GetProperty("submittedBy").GetString() == "analyst-private-note");
        Assert.DoesNotContain(candidNote, explained.GetRawText(), StringComparison.OrdinalIgnoreCase);
    }

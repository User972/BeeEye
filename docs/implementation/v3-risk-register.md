...
| R-11 | Accessibility regression | Medium | Medium | S1 added skip link, landmarks, `aria-current`, focus-visible, reduced motion; S12 adds automated scans (vitest-axe on components + @axe-core/playwright on every route, both themes, zero serious/critical); V3-DS-007 fixes the drawer focus trap | Mitigating |
